...
Font *font_load(Gpu gpu, Arena *arena, char *path, float size) {
    ...
    for(i32 codepoint = FONT_CODEPOINT_RANGE_START; codepoint <= FONT_CODEPOINT_RANGE_END; ++codepoint) {
        ...
        stbtt_MakeCodepointBitmap(font, (unsigned char *)bitmap8.data,
                                  (i32)bitmap8.w, (i32)bitmap8.h, (i32)bitmap8.w, scale, scale, codepoint);
...
R2 font_size_text(Font *font, const char *text, f32 x, f32 y) {
    ...
    for(sz i = 0; i < len; ++i) {
        ...
        if (text[i+1]) {
            i32 kerning = stbtt_GetCodepointKernAdvance(&font->stbfont, (i32)text[i],(i32)text[i+1]);
...
void font_draw_text(Gpu gpu, Font *font, const char *text, f32 x, f32 y, V3 color) {
    ...
    for(sz i = 0; i < len; ++i) {
        ...
        gpu_draw_quad_texture_tinted(gpu, pos_x, pos_y, (f32)glyph->bitmap.w, (f32)glyph->bitmap.h, 0, glyph->texture, color);
